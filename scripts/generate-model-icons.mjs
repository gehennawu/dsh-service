...
/** 抽取 SVG 中所有硬编码色值（fill / stop-color），用于对比度判定。支持 3 位与 6 位 hex 及基本具名色。 */
function hardcodedColors(svg) {
  const found = new Set()
  for (const m of svg.matchAll(/(?:fill|stop-color)="((?:#[0-9a-fA-F]{3,8})|[a-zA-Z]+)"/g)) {
    ...
    if (/^#[0-9a-fA-F]{6}$/.test(hex)) found.add(hex)
  }
  return [...found]
}
...
const CUSTOM_SVG = {
  ...
  /**
   * Kimi「K·」品牌标（官方 App 徽标全彩版）——**替换**图标库的 `kimi-color`。
   *
   * 为什么需要替换：
   * 图标库原生的 `kimi-color.svg` 把字母 K 写死为 `fill="#fff"`，且没有背景板。
   * 在浅色主题（按钮背景为白色）下，白色字母 K 会与白色背景同色融为一体完全隐形，只剩下一颗蓝色浮点。
   *
   * 修复方案：
   * 采纳 Kimi 官方 Brand Guidelines（App Icon 规范），使用官方深色圆角方块（#121316）
   * 作为底板衬托白 K 与天蓝水滴点（#1783FF）。这样在浅色与深色模式下，白色字母 K 均在深色底板内
   * 形成清晰衬托，绝不隐形，同时完整保留标志性的天蓝水滴彩色。
   */
  kimi:
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40">' +
    '<rect width="40" height="40" rx="10" fill="#121316"/>' +
    '<path d="M31.6957 7.11414C32.8305 7.11414 33.7505 8.0341 33.7505 9.16893V9.16893C33.7505 10.3038 32.8305 11.2237 31.6957 11.2237L29.8826 11.2237C29.7491 11.2237 29.6409 11.1155 29.6409 10.982L29.6409 9.16893C29.6409 8.0341 30.5609 7.11414 31.6957 7.11414V7.11414Z" fill="#1783FF"/>' +
    '<path d="M20.1757 19.0798L27.9302 11.3864C28.0761 11.2413 27.9939 10.9498 27.8056 10.9498H23.633C23.5877 10.9498 23.5438 10.9683 23.5084 11.0038L15.1533 19.2911C15.023 19.4192 14.8304 19.305 14.8304 19.0998V11.1951C14.8304 11.0593 14.7411 10.9498 14.6321 10.9498H11.7569C11.6478 10.9498 11.5586 11.0593 11.5586 11.1951V28.2387C11.5586 28.3745 11.6478 28.484 11.7569 28.484H14.6321C14.7411 28.484 14.8304 28.3745 14.8304 28.2387V24.7661C14.8304 24.6921 14.8573 24.6211 14.904 24.5748L17.4945 22.0047C17.5568 21.943 17.6432 21.9337 17.7126 21.98L24.6414 27.0786C25.7759 27.8423 27.0436 28.3112 28.331 28.4501C28.4458 28.4624 28.5449 28.3483 28.5449 28.2048V24.9343C28.5449 24.8093 28.4698 24.7075 28.3693 24.6921C27.6115 24.5733 26.8722 24.2802 26.2037 23.8297L20.2054 19.487C20.0808 19.4037 20.0652 19.1893 20.1757 19.0798Z" fill="#FFFFFF"/>' +
    '</svg>',

  /**
   * CLIProxyAPI（CPA）品牌标
   *
   * 外框为四个尖角内凹的菱形曲线（astroid-like curved diamond），中心为水平镜像反转的 OpenAI 花瓣漩涡徽标。
   * 单色档（c: 0），遵循主题文字色通过 mask 渲染。
   */
  cliproxy:
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-19 -19 38 38">' +
    '<path fill="none" stroke="currentColor" stroke-width="1.9" stroke-linejoin="round" d="M 0 -18 Q -5.8 -5.8 -18 0 Q -5.8 5.8 0 18 Q 5.8 5.8 18 0 Q 5.8 -5.8 0 -18 Z"/>' +
    '<g transform="scale(-0.835, 0.835) translate(-12, -12)">' +
 ...
 * 它的墨迹面积明显更大，真机看就偏大。收紧 viewBox 已经解决了
...
for (const [provider, slug] of ENTRIES) {
  ...
  const isCustomColor = customSvg !== undefined && /(?:fill|stop-color)="(?!currentColor|none)(#[0-9a-fA-F]{3,8}|[a-zA-Z]+)"/i.test(customSvg)
