...
`
    ...
    const zh = {
      ...
      'plugin.compat.category.service-removed': '客户端服务已移除',
      ...
      'plugin.compat.reason.settings-scope': 'DSH 0.1.7-alpha.1 配置面换代，客户端 settingsScope 服务被 configForms 取代',
      'plugin.compat.impact.settings-scope': '文本扫描分不清接入形态：声明式静态 inject 或未探测的直读会挂起客户端激活；ctx.inject 回调形态只是回调不再执行、插件本体照常运行（按较轻形态列入提示档）',
      'plugin.compat.advice.settings-scope': '按能力探测动态接入 configForms 或 settingsScope，不要把任一服务写进静态 inject',
      'plugin.compat.reason.settings-register': 'DSH 0.1.7-alpha.1 宿主配置迁移到 Profile 承载的 SettingsForms，移除旧 register 方法',
      'plugin.compat.impact.settings-register': '命名空间注册调用失效（异常被吞则静默），功能开关停留在默认值、无法持久化',
      'plugin.compat.advice.settings-register': '探测旧 register 是否存在：老宿主照旧注册，新宿主改声明 plugin.Config（volatile 字段）',
      'plugin.compat.reason.settings-get': 'DSH 0.1.7-alpha.1 宿主 settings 服务移除旧 get 读取入口',
      'plugin.compat.impact.settings-get': '读取宿主命名空间配置（如自定义模型渠道）拿到空值，相关功能静默退化',
      'plugin.compat.advice.settings-get': '改读 describe 描述符或 plugin.Config 声明值，并保留旧读取路径的能力探测降级',
      ...
      'plugin.compat.break.settings-scope': '引用了已移除的客户端设置服务 settingsScope（0.1.7-alpha.1 起官方配置面改由 configForms 提供；回调形态的旧引用只是不再执行、插件本体照常运行，声明式静态 inject 才会挂起激活）',
      'plugin.compat.break.settings-register': '调用已移除的设置注册方法 settings.register（0.1.7-alpha.1 起宿主 settings 服务重构为 SettingsForms，注册调用失效，功能开关停留在默认值）',
      'plugin.compat.break.settings-get': '调用已移除的配置读取方法 settings.get（0.1.7-alpha.1 起宿主 settings 服务不再提供旧读取接口，自定义配置读取退化为空）',
      ...
      'backup.row.version': '备份时 DSH 版本：{version}',
      'backup.empty': '还没有备份。',
      'backup.delete': '删除',
      'backup.confirm': '确认删除',
      ...
      'backup.integrity.sourceVersion': '备份时 DSH 版本：{version}',
    ...
    const en = {
      ...
      'plugin.compat.kind.soft': 'References a retired interface',
      ...
      'plugin.compat.issue.soft': '{count} plugin(s) reference a retired interface',
      ...
      'plugin.compat.category.service-removed': 'Client service removed',
      ...
      'plugin.compat.reason.settings-scope': 'The client config surface was replaced in DSH 0.1.7-alpha.1 — the settingsScope client service was superseded by configForms',
      'plugin.compat.impact.settings-scope': 'Text scanning cannot tell reference shapes apart: a declarative static inject or an unguarded direct read keeps the client activation pending, while a ctx.inject callback merely stops firing and the plugin keeps running (reported at the milder tier)',
      'plugin.compat.advice.settings-scope': 'Probe for configForms or settingsScope dynamically instead of listing either service in static inject',
      'plugin.compat.reason.settings-register': 'Host configuration moved to Profile-backed SettingsForms in DSH 0.1.7-alpha.1, dropping the legacy register method',
      'plugin.compat.impact.settings-register': 'Namespace registration silently stops taking effect, so feature switches stay at their defaults and never persist',
      'plugin.compat.advice.settings-register': 'Probe for the legacy register first: keep registering on older hosts, and declare plugin.Config (volatile fields) on new hosts',
      'plugin.compat.reason.settings-get': 'The host settings service in DSH 0.1.7-alpha.1 dropped the legacy get read entry point',
      'plugin.compat.impact.settings-get': 'Reading host namespace config (such as custom model channels) returns empty and related features silently degrade',
      'plugin.compat.advice.settings-get': 'Read describe descriptors or plugin.Config values instead, keeping a capability-probe fallback to the legacy read path',
      ...
      'plugin.compat.break.settings-scope': 'References the removed client settings service settingsScope (the official config surface moved to configForms in 0.1.7-alpha.1; callback-shaped references merely stop firing while the plugin keeps running, only a declarative static inject hangs activation)',
      'plugin.compat.break.settings-register': 'Calls the removed settings registration method settings.register (the host settings service became SettingsForms in 0.1.7-alpha.1; registration no longer takes effect and switches stay at their defaults)',
      'plugin.compat.break.settings-get': 'Calls the removed config read method settings.get (the host settings service no longer offers the legacy read API in 0.1.7-alpha.1, so custom config reads degrade to empty)',
      ...
      'backup.row.version': 'DSH version at backup time: {version}',
      ...
      'backup.integrity.sourceVersion': 'DSH version at backup time: {version}',
