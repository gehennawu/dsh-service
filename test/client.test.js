...
test('service panel puts versions first and renders switchable provider-prefixed usage charts by project', async () => {
  ...
  assert.match(renderer.text('settings.section'), /精确数据.*Token.*[0-9,]+/)
  visibleSegment.props.onMouseLeave()
  await renderer.flush()
  assert.doesNotMatch(renderer.text('settings.section'), /精确数据/)
...
test('service panel uses distinct cards, display surfaces, and semantic action colors', async () => {
  ...
  assert.equal(tabs.props.style.borderBottom.includes('solid'), true)
  assert.equal(renderer.findButton('概览').props.style.borderBottom.includes('solid'), true)
  ...
  assert.match(overviewSurface.props.style.background, /var\(--|#fff|rgb/)
  assert.doesNotMatch(overviewSurface.props.style.background, /rgba/)
  assert.match(overviewSurface.props.style.border, /solid/)
  await renderer.findButton('健康诊断').props.onClick()
  await renderer.flush()
  const healthRegion = renderer.findByTestId('health-diagnostics-region')
  assert.match(healthRegion.props.style.border, /solid/)
  assert.match(healthRegion.props.style.background, /var\(--|#fff/)
  const healthAction = renderer.findButton('立即健康检查')
  assert.equal(healthAction.props['data-variant'], 'neutral')
  assert.equal(healthAction.props.style.background, 'var(--dsh-control, #ffffff)')
  assert.doesNotMatch(healthAction.props.style.background, /rgba/)
  await renderer.findButton('模型统计').props.onClick()
  await renderer.flush()
  ...
  assert.equal(createBackup.props['data-variant'], 'primary-outline')
  assert.equal(createBackup.props.style.background, 'var(--dsh-control, #ffffff)')
  await renderer.findButton('重启').props.onClick()
  await renderer.flush()
  const restartRegion = renderer.findByTestId('restart-region')
  assert.match(restartRegion.props.style.border, /solid/)
  assert.match(restartRegion.props.style.background, /var\(--|#fff/)
  assert.doesNotMatch(restartRegion.props.style.background, /rgba/)
  const restart = renderer.findButton('重启 dsh web')
  assert.equal(restart.props['data-variant'], 'danger-outline')
  assert.notEqual(restart.props.style.background, '#d33')
})
