...
- **Automatic update status**: when Settings opens, checks DSH and plugin release states independently from fixed npm registry URLs, so one failure does not contaminate the other. Because `@gehennawu/dsh-service` is not currently published to npm, its HTTP 404 is shown as “No published version is available to check,” while DSH still reports normally. Both rows provide release links. The Host caches successful/partial results for 10 minutes and complete failures for 1 minute.
...
- **Five-tab service control**: Settings is split into Overview, Health diagnostics, Model statistics, Backup maintenance, and Restart line-style tabs. It uses a DSH-native-like underline selection and one content layer instead of nested background cards. Ordinary actions share a neutral solid surface and thin border; primary actions such as creating a backup use a low-saturation brand outline; repair/delete actions use a soft red danger outline, while **Restart dsh web** and final destructive confirmations use solid red with white text. Overview contains versions, update states, existing container runtime metrics, and last-24-hour model/tool errors; deep diagnostics and file-permission inspection/repair live in Health diagnostics.
- **Simplified model-statistics layout**: project selection uses compact line tabs. The main chart is a fixed seven-day stacked view of input, output, and cache tokens with blue/orange/teal legend colors. The chart includes a K/M-abbreviated vertical axis, a dated horizontal baseline anchored at the bottom of the chart, and low-contrast grid lines; bars use the available height above that baseline. Hovering a colored segment highlights it and shows the date, type, and exact token value beside the pointer. The chart, legend, today/seven-day summaries, cache hit rate, and provider/model ranking share one continuous statistics region. Today and seven-day summaries both show input, output, cache tokens, successful steps, and cache hit rate. Provider/model rows are sorted by seven-day step count and show only the top three by default; remaining models expand with `▸` and collapse with `▾`.
