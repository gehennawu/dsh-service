...
| Stage | Status | Scope |
| --- | --- | --- |
| Current `0.10.0` | ✅ Feature complete | Separate automatic DSH/plugin update states, cached checks, and automatic diagnostics on Health-tab entry are implemented and tested |
...
- **Automatic update status**: when Settings opens, reads `latest` for both `@deepseek-ai/dsh` and `@gehennawu/dsh-service` from fixed npm registry URLs. Overview independently shows each current version, whether it is current, and a releases link, with no manual check button. The Host caches successful results for 10 minutes and failures for 1 minute to avoid repeated registry access.
...
- **Five-tab service control**: Settings is split into Overview, Health diagnostics, Model statistics, Backup maintenance, and Restart line-style tabs. It uses a DSH-native-like underline selection and one content layer instead of nested background cards. Ordinary actions share a neutral solid surface and thin border; primary actions such as creating a backup use a low-saturation brand outline; repair/delete/restart use a soft red danger outline, while only final confirmations use solid red. Overview contains only versions, update controls, and existing container runtime metrics; deep diagnostics and file-permission inspection/repair live in Health diagnostics.
- **Simplified model-statistics layout**: project selection uses compact line tabs. The main chart is a fixed seven-day stacked view of input, output, and cache tokens with blue/orange/teal legend colors. The chart includes a K/M-abbreviated vertical axis, a dated horizontal baseline, and low-contrast grid lines. Hovering a colored segment highlights it and shows the date, type, and exact token value beside the pointer. The chart, legend, today/seven-day summaries, cache hit rate, and provider/model ranking share one continuous statistics region. Today and seven-day summaries both show input, output, cache tokens, successful steps, and cache hit rate. Provider/model rows are sorted by seven-day step count and show only the top three by default; remaining models expand with `▸` and collapse with `▾`. Error areas continue to use whitespace and dividers.
...
- **Linux file permissions**: automatically performs a shallow check of DSH_HOME and all workspace roots inside the Health card, with paths collapsed by default. A manual deep check recursively validates ownership, directory mode 755, and regular-file mode 644; for `$DSH_HOME/.credentials.yaml` it follows the credential plugin's actual contract — group/other permission bits must be clear (owner-only; 0400/0600 and any other owner-only mode is compliant, not literally 600 only) — and that exception applies to this one file alone, never to same-named files inside workspaces. Nested workspaces (for example `/workspace` registered alongside its subdirectories) are traversed once under the outermost root and are not counted twice. Repair still requires two-step confirmation; it repairs general entries first and then explicitly restores the credential file to 600 so the DSH credential plugin cannot reject startup because of overly broad access. The section is hidden outside Linux. If DSH runs as root in a container, the target is still root, so this cannot downgrade ownership to an unprivileged user.
