...
[![Version](https://img.shields.io/badge/version-1.9.4-3b82f6.svg?style=flat-square)](package.json)
...
- Inline "What's new": the host fetches the release body from the GitHub Releases API and the client renders it in place (plain Markdown — no iframe, no navigation), with the publish date and a pre-release tag; a missing Release or a failed read each get their own message
