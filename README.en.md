...
| Stage | Status | Scope |
| --- | --- | --- |
| Current `0.9.0` | ✅ Feature complete | Native-style five-tab Settings layout, Overview/Health separation, and simplified model-statistics presentation are implemented and tested |
...
- **Health panel and deep diagnostics**: shows uptime, memory RSS, live/persisted sessions, active agents, and background jobs, with lightweight metrics refreshed every 5 seconds. File-permission inspection, deep scan, and repair live in Health diagnostics. A prominent health alert appears when the shallow check or manual diagnostics reports a warning/error. Results translate internal counts into readable explanations such as “Backup directory is available; no backups exist yet” or “Found N file or directory permission anomalies” rather than exposing formats such as `0:0`.
