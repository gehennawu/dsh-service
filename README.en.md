...
- **Simplified model-statistics layout**: project selection uses compact line tabs. The main chart is a fixed seven-day stacked view of input, output, and cache tokens with blue/orange/teal legend colors. The chart includes a K/M-abbreviated vertical axis, a dated horizontal baseline, and low-contrast grid lines. Hovering a colored segment highlights it and shows the date, type, and exact token value beside the pointer. Provider/model rows are sorted by seven-day step count and show only the top three by default; remaining models can be expanded and collapsed again. Today/seven-day summaries, cache hit rate, and collapsed error areas use whitespace and dividers instead of nested boxes.
