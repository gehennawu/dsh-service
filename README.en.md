...
- **Linux file permissions**: automatically performs a shallow check of DSH_HOME and all workspace roots inside the Health card, with paths collapsed by default. A manual deep check recursively validates ownership, directory mode 755, and regular-file mode 644. Only `$DSH_HOME/.credentials.yaml` receives the credential exception: its group/other bits must be clear (owner-only, so modes such as 0400 and 0600 are compliant); same-named workspace files remain ordinary 644 files. Nested workspaces (for example `/workspace` registered alongside its subdirectories) are traversed once under the outermost root and are not counted twice. Repair still requires two-step confirmation; it repairs general entries first and then normalizes the actual `$DSH_HOME/.credentials.yaml` to 600 so the DSH credential plugin cannot reject startup because of overly broad access. A deep scan after repair should introduce no new anomaly. The section is hidden outside Linux. If DSH runs as root in a container, the target is still root, so this cannot downgrade ownership to an unprivileged user.
