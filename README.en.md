...
[![Version](https://img.shields.io/badge/version-1.4.10-3b82f6.svg?style=flat-square)](package.json)
...
- Edge gestures: swipe rightward from the left screen edge to open the sidebar drawer, swipe leftward (right-to-left) to close it while open; swipe leftward from the right screen edge to open the right sidebar (when dsh-better-sidebar is installed), swipe rightward (left-to-right) to close it while open, and the gesture stays inert when no right-sidebar plugin is present. The detection band is kept narrow to avoid browser system edge gestures, and horizontal scrolling inside editors never misfires it
