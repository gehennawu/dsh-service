...
- **Simplified model-statistics layout**: project selection uses compact line tabs. The main chart is a fixed seven-day stacked view of input, output, and cache tokens with blue/orange/teal legend colors. Hovering a colored segment highlights it and shows the date, type, and exact token value. Token summaries use K/M abbreviations, while model steps use a count unit. Today/seven-day summaries, provider/model rows, cache hit rate, and collapsed error areas use whitespace and dividers instead of nested boxes.
