...
- **Linux file permissions**: automatically performs a shallow check of DSH_HOME and all workspace roots inside the Health card, with paths collapsed by default. A manual deep check recursively validates ownership, directory mode 755, regular-file mode 644, and the sensitive `.credentials.yaml` exception at mode 600. Nested workspaces (for example `/workspace` registered alongside its subdirectories) are traversed once under the outermost root and are not counted twice. Repair still requires two-step confirmation; it repairs general entries first and then explicitly restores `$DSH_HOME/.credentials.yaml` to 600 so the DSH credential plugin cannot reject startup because of overly broad access. The section is hidden outside Linux. If DSH runs as root in a container, the target is still root, so this cannot downgrade ownership to an unprivileged user.
