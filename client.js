      ...
      function SubagentSection() {
        ...
        return React.createElement('div', { 'data-testid': 'subagent-section', style: cardStyle },
          ...
          mode === 'custom' ? React.createElement('div', { 'data-testid': 'subagent-custom', style: { marginTop: '10px', display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' } },
            ...
            mode === 'custom' && model !== '' ? React.createElement('span', { style: { fontSize: '12px', color: 'var(--dsw-alias-label-secondary)' } }, translate('subagent.reasoningEffort')) : null,
            mode === 'custom' && model !== '' ? React.createElement('select', { 'data-testid': 'subagent-reasoning-effort', value: reasoningEffort, disabled: saving || effortOptions.length === 0, onChange: (event) => setReasoningEffort(event.target.value), style: selectStyle },
              React.createElement('option', { value: '' }, translate('subagent.reasoningEffort.default')),
              ...effortOptions.map((option) => React.createElement('option', { key: option.id, value: option.id, ...(option.description !== undefined ? { title: option.description } : {}) }, option.name))) : null,
            mode === 'custom' && model !== '' && effortOptions.length === 0 ? React.createElement('span', { 'data-testid': 'subagent-reasoning-effort-unavailable', style: { fontSize: '12px', color: 'var(--dsw-alias-state-warn-primary)' } }, translate('subagent.reasoningEffort.unavailable')) : null) : null,
