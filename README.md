...
- **查看**：统一列表展示会话（运行中 / 冷会话 / 已归档），行内标状态徽章、工作区、事件数、文件体积；**默认停在「仅归档」视图**，全部 / 仅归档 / 已删除三个筛选各自**首次**按需向宿主拉取对应子集并缓存（模块级缓存：切换筛选零请求、**关掉面板再打开秒显缓存 + 后台静默刷新一次保鲜**，页面刷新才清零），「刷新」按钮可强制重拉当前视图；文件体积不随列表下发、行内按需懒加载（模块级 + 宿主进程内存双层缓存：刷新浏览器 / 重开面板直接复用，删除时失效）；详情按事件卡片分页浏览（宿主单槽位快照缓存：翻页/重进详情零重复读取，live 会话 30 秒内保鲜），连续系统事件默认折叠为计数块、点击展开明细
- **导出**：一键下载官方完整 ZIP（含子代理与附件），复用官方导出链路，宿主不自己拼包
- **归档**：归档后从官方侧栏隐藏（官方行为）；面板内明确标注「归档为单项操作」，官方不支持恢复
- **内容搜索**：对话全文语义搜索（大小写不敏感、空白灵活），跨会话命中列表 → 单会话命中事件视图；可限定仅搜归档区
- **删除**：非运行中会话可删除，两段式确认（先展示后果清单：会话 id / 标题 / 工作区 / 文件体积 / 将从官方侧栏消失）；已删除记录在「已删除」筛选下可见（只读）
- 入口：顶部标签「技能与维护」组「会话管理」胶囊（默认开），设置页左列入口可选（默认关）
