...
- 独立的「额度查询」标签：列出全部已配置供应商，已适配的每个窗口显示百分比、独立进度条，重置时间单独一行；未适配的灰行内可直接选择适配类型并即时保存
...
- 内置适配：**OpenCode Go**（`{baseURL}/usage`）、**智谱 GLM Coding Plan / zai-coding-cn**（官方监控端点 `quota/limit`，含 5 小时滚动 Token、每周 Token、MCP 月度配额三个窗口，5 小时窗口空闲时与官网一致地不显示重置时间）、**OpenRouter**（credits 已用%）、**Kimi/Moonshot** 与 **硅基流动**（人民币余额）；上游瞬时网络错误自动重试，智谱双域候选链自动切换；供应商与适配类型的对应关系保存在 `DSH_HOME/dsh-service-quota.json`，已知服务商按 baseURL 自动识别适配（如 opencode.ai、bigmodel.cn），无需手选；未适配的供应商不会被发起任何上游请求，也可在配置文件写 `"<provider>": null` 显式停用。智谱的重置卡暂无 API Key 可查的接口，可在「额度查询」标签内点各供应商行的「重置卡…」直接填写次数与到期日期（数据存入同一配置文件），过期自动标注
