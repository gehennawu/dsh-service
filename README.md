...
- **Linux 文件权限**：打开面板时在健康卡片内自动浅检查 DSH_HOME 与全部工作区根目录并直接显示异常摘要，路径详情默认折叠；手动深度检查递归扫描属主、目录 755、普通文件 644，并将 `.credentials.yaml` 作为敏感例外要求 600。嵌套工作区（如 `/workspace` 与其子目录同时注册）只按外层根扫描一次，不重复计数。修复仍需两段式确认，先处理一般目录/文件，再显式恢复 `$DSH_HOME/.credentials.yaml` 为 600，避免 DSH 凭据插件因权限过宽而拒绝启动。非 Linux 不显示此功能。容器内若以 root 运行 DSH，目标仍是 root，因此该修复不能降权为普通用户。
