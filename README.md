...
- 「技能」标签按**自动加载 / 仅手动调用 / 完全停用**三区展示全部本地技能（项目 `.dsh`、项目 `.agents`、用户 `~/.dsh/skills`、`$DSH_AGENTS_HOME` 与 `$DSH_BUNDLED_SKILL_DIR` 根，一层深度），支持名称过滤与来源徽标；同名遮蔽（低 rank 优先）与被遮蔽副本均有标注，内置目录只读展示
...
- 带 camelCase 旧版调用键（如 `disableModelInvocation`）的条目会被官方解析器整条剔除：面板给出 ⚠ 提示和一键修复，按语义换算为规范键
- ✨「AI 补全说明」：从已配置模型中任选一个（记住上次选择），由宿主用固定模板调用该模型生成描述与用法草稿——描述跟随 SKILL 正文语言、限一句话路由句式；草稿以新旧对照预览，显式确认后才写入文件。「已注释」标记存于 `DSH_HOME/dsh-service-skills-index.json`（正文哈希比对）：改正文会重新变回待补全，只改描述或开关不会
- 一键批量补全：自动圈出可写根下未注释或正文有变的技能，先展示候选数/预计发送量/跳过清单，确认后顺序执行并显示进度，单条失败不影响批次，可随时取消；插件绝不自动发起任何模型调用
