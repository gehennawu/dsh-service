...
| 阶段 | 状态 | 内容 |
| --- | --- | --- |
| 当前版本 `0.11.0` | ✅ 功能完成 | 版本链接、模型 Token 展示、备份导入导出、/restart 命令和权限修复交互已实现并通过自动化测试 |
...
- **自动更新状态**：设置页打开时从固定 npm registry 地址独立读取 DSH 与插件版本状态，单项失败不会污染另一项；版本号本身链接到对应 GitHub Releases。Host 对成功/部分成功结果缓存 10 分钟，对完全失败结果缓存 1 分钟。
...
- **备份管理**：在 `$DSH_HOME/backups/` 创建会话、配置和插件 profile 清单归档；按日期从新到旧列出，10 条以内直接展示，超过 10 条才折叠。每条记录都有右侧删除和浏览器导出按钮，支持选择 `.tar.gz` 文件导入；删除仍需两段式确认。备份不限份数且不会自动清理，磁盘占用由用户自行管理；归档不包含凭据或 `node_modules`。
