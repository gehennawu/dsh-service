...
| 阶段 | 状态 | 内容 |
| --- | --- | --- |
| 当前版本 `0.9.0` | ✅ 功能完成 | 五标签原生设置页风格、概览/健康拆分和简化模型统计布局已实现并通过自动化测试 |
...
- **健康面板与完整诊断**：显示 uptime、内存 RSS、存活/持久化会话、活跃 Agent 和后台任务；轻量指标每 5 秒刷新。文件权限检查与查看/深检/修复操作统一收纳在健康诊断标签内；自动浅检或手动诊断出现 warning/error 时显示醒目的「健康提醒」。诊断结果会把内部计数转换为可读说明，例如「备份目录可用，当前暂无备份」或「发现 N 个文件或目录权限异常」，不直接展示 `0:0` 等内部格式。
