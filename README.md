...
| 阶段 | 状态 | 内容 |
| --- | --- | --- |
| 当前版本 `0.8.0` | ✅ 功能完成 | 四标签服务控制、功能故障提醒和高区分度展示面已实现并通过自动化测试 |
...
- **四标签服务控制**：设置页拆分为「概览」「模型统计」「备份维护」「重启」四个标签，默认打开概览，只渲染当前功能区，减少纵向堆叠。标签内容使用更深的独立背景、实色边框与轻微阴影，指标、图表、诊断结果和错误列表再使用更高对比度的二级展示面。检查/刷新使用蓝色信息按钮，主要选择使用品牌色，修复/删除/重启保留危险色。
- **功能故障提醒**：健康/权限、统计 RPC、备份操作或重启流程发生功能故障时，对应标签显示 `⚠`，并在服务控制顶部汇总受影响标签。没有备份不算故障；最近 24 小时的模型报错和工具报错属于统计数据，也不会触发标签或顶部提醒。
