...
[![Version](https://img.shields.io/badge/version-0.33.0-3b82f6.svg?style=flat-square)](package.json)
...
面向自托管 DSH Web 的服务控制与运维插件：安全重启、版本管理与一键升级、健康诊断、模型用量统计、额度查询、备份管理、任务通知、技能管理、会话管理与 Linux 文件权限维护。
...
设置页「服务控制」面板十个标签：**概览 · 通知 · 健康诊断 · 模型统计 · 额度查询 · 备份维护 · 技能 · 子代理 · 会话管理 · 重启**；重启、额度查询、技能、子代理、会话管理可另行开启**设置页左列快捷入口**（默认关闭）。
