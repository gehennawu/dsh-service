...
| 阶段 | 状态 | 内容 |
| --- | --- | --- |
| 当前版本 `0.6.0` | ✅ 功能完成 | v0.3–v0.6 路线功能均已实现并通过自动化测试 |
...
- **健康面板与完整诊断**：显示 uptime、内存 RSS、存活/持久化会话、活跃 Agent 和后台任务；轻量指标每 5 秒刷新。手动健康检查另外验证会话存储、工作区注册表、DSH_HOME、备份目录、tar 和根目录权限。
...
- **模型报错统计**：从 `llm/retry` 与最终失败回合中提取 provider 错误，按 `provider/model + code + HTTP status` 归类，分别展示历史累计和滚动最近 24 小时，并按次数从高到低排列；取消、最大 Token 和 Tool 错误不计入。
...
- **Linux 文件权限**：打开面板时自动浅检查 DSH_HOME 与全部工作区根目录并直接显示异常摘要，路径详情默认折叠；手动深度检查递归扫描属主、目录 755、文件 644 并返回有限异常样例。修复仍需两段式确认。非 Linux 不显示此功能。容器内若以 root 运行 DSH，目标仍是 root，因此该修复不能降权为普通用户。
...
- 模型用量索引仅保存 session revision/watermark 与按日期、项目、provider/model 聚合的数字，不保存消息、Prompt、Tool 参数、密码、私钥或其他凭据；固定写入 `$DSH_HOME/dsh-service-usage-index.json` 并使用 0600 权限。
