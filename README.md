...
- 热力块右上角可切到**「小时」打卡图**：7 行（周一–周日）× 24 列（0–23 时），格 = 该时段累计 token，回答「几点最忙」；同一批小时桶换个维度折出，不新增持久化字段。24 列在统计区宽度内用 16px 格（20px 会溢出）
- **已统计的用量永久保留**：删除会话或删除项目文件夹都不会扣减历史，总量与热力图始终是「索引建立以来的完整历史」；索引版本升级时这些条目原样带过，不会被重建清空
