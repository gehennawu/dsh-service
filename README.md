...
- **简化模型统计布局**：项目切换使用线型小标签，主图固定为近 7 天输入/输出/缓存 Token 堆叠柱图，并用蓝/橙/青图例区分结构。图表带有 K/M 缩写的纵轴刻度、横轴日期基线和低对比度网格线；鼠标移入颜色区段会高亮该区段，并在鼠标旁显示日期、类型和精确 Token 数。模型步骤使用「次」，provider/model 明细按近 7 天步骤数降序，默认只显示前三，其他模型可展开并再次收起。今日/近 7 天汇总、缓存命中率和折叠错误区域以留白与分隔线组织，不再层层套框。
