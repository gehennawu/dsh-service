...
[![Version](https://img.shields.io/badge/version-0.39.0-3b82f6.svg?style=flat-square)](package.json)
...
- 供应商卡片分区：身份 → 核心余额（余额类窗口前置）→ 最紧窗口（使用率最高的窗口带徽标）→ 重置记录；**高级配置**（凭据填写、类型切换、手动重置录入）默认折叠，按卡展开
